  ...
  environment {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                cleanWs()
                git branch: 'develop', url: 'https://github.com/Shopping-App-Services/product-catalog-service.git'
            }
        }
            ...
            steps {
                script {
                  ...
                  withDockerRegistry(credentialsId: '20226572-5e4c-4db0-ad81-6762251b3d09', toolName: 'docker') {
                        sh 'ls -latr'
                        sh "docker build -t product-catalog-service ."
                        sh "docker tag product-catalog-service nitesh2611/product-catalog-service:latest "
                        sh "docker push nitesh2611/product-catalog-service:latest "
                    }
                }
            }
        }
